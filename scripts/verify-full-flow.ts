...
async function main() {
    ...
    // Save payer key for CLI use
    const fs = require('fs');
    const { execSync } = require('child_process');
    fs.writeFileSync('payer.json', `[${payer.secretKey.toString()}]`);
    ...
    try {
        ...
        const configureCmd = `spl-token --program-id TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb --url ${LOCALNET_URL} --owner payer.json --fee-payer payer.json configure-confidential-transfer-account ${wrappedMint}`;
